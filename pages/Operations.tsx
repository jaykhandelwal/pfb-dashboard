      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <div className="flex border-b border-slate-200">
          <button
            ...
            className={`flex-1 py-3 md:py-4 text-center font-medium transition-colors flex items-center justify-center gap-2 text-sm md:text-base ${
           ...
           <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col">
              ...
              <div className="flex-1 overflow-y-auto p-4 md:p-6">
                          ...
                          <div 
                            ...
                            className={`flex justify-between items-center p-3 rounded-lg border ${
                               isZero 
                                 ...
                                 : (type === TransactionType.CHECK_OUT ? 'bg-emerald-50 border-emerald-100' : 'bg-blue-50 border-blue-100')
                            }`}
                          >
                             <div className="flex items-center gap-3 overflow-hidden">
                                <div className={`w-2 h-2 rounded-full flex-shrink-0 ${isZero ? 'bg-red-400' : (type === TransactionType.CHECK_OUT ? 'bg-emerald-500' : 'bg-blue-500')}`} />
                                <span className={`font-medium truncate ${isZero ? 'text-red-900' : 'text-slate-700'}`}>{sku.name}</span>
                             </div>
                             
                             <div className="text-right flex-shrink-0 ml-2">
                                <div className={`font-mono font-bold leading-none ${isZero ? 'text-red-500' : (type === TransactionType.CHECK_OUT ? 'text-emerald-700' : 'text-blue-700')}`}>
                                   {qty} <span className="text-xs font-sans font-normal opacity-60">pcs</span>
                                </div>
                                <div className={`text-[10px] mt-0.5 ${isZero ? 'text-red-400' : 'text-slate-500'}`}>
                 ...
                 <div className="mt-4 p-3 bg-slate-50 rounded-lg text-xs text-slate-500 flex items-start gap-2">
                    ...
                    <p>Items highlighted in <span className="text-red-600 font-bold">Red</span> have 0 quantity. Please ensure you haven't missed anything.</p>
                 </div>
              </div>
