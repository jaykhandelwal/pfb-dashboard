...
import { SalesPlatform, OrderItem, MenuItem, RewardResult, Order, Customer } from '../types';
...
const Orders: React.FC = () => {
            ...
            <div key={order.id} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow flex flex-col gap-3 relative group">
                <div className="flex justify-between items-start">
                    <div>
                        ...
                        <h4
                            className={`font-bold text-sm ${order.customerId ? 'text-indigo-600 cursor-pointer hover:underline' : 'text-slate-700'}`}
                            onClick={() => {
                                if (order.customerId) {
                                    const customer = customers.find(c => c.id === order.customerId);
                                    if (customer) setSelectedCustomerForModal(customer);
                                }
                            }}
                        >
    ...
    return (
        <div className="h-[calc(100vh-6rem)] flex flex-col">
            ...
            {/* --- CUSTOMER DETAILS MODAL (DYNAMIC) --- */}
