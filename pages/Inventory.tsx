...
const Inventory: React.FC = () => {
  const { skus, transactions, addBatchTransactions } = useStore();
  ...
  const calculateDiscrepancies = () => {
    ...
    skus.forEach(sku => {
      const systemStock = stockLevels[sku.id]?.balance || 0;
      ...
      <div className="mb-8 flex flex-col xl:flex-row xl:items-end justify-between gap-4">
        ...
        <div className="flex bg-slate-100 p-1 rounded-lg overflow-x-auto">
           <button
             ...
             className={`px-4 md:px-6 py-2 rounded-md text-sm font-semibold transition-all whitespace-nowrap ${
               activeTab === 'view' 
           ...
           <button
             ...
             className={`px-4 md:px-6 py-2 rounded-md text-sm font-semibold transition-all flex items-center gap-2 whitespace-nowrap ${
               activeTab === 'restock' 
