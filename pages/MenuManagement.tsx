                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                     ...
                     <div>
                        ...
                        <input
                           ...
                           className={`w-full border rounded-lg px-3 py-2 focus:ring-2 focus:ring-emerald-500 focus:outline-none font-mono text-sm ${menuItems.some(i => i.id === currentItem.id) ? 'bg-slate-100 text-slate-500 border-slate-200 cursor-not-allowed' : 'border-slate-300 bg-white'
                              }`}
                  ...
                  <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 mt-4">
                     ...
                     <div className="flex gap-1 bg-slate-200 p-1 rounded-lg mb-4">
                        <button
                           ...
                           className={`flex-1 py-1.5 text-xs font-bold rounded-md transition-all ${recipeTab === 'FULL' ? 'bg-white text-emerald-700 shadow-sm' : 'text-slate-500 hover:text-emerald-700'
                              }`}
                        >
                           Full Plate Recipe
                        </button>
                        <button
                           ...
                           className={`flex-1 py-1.5 text-xs font-bold rounded-md transition-all ${recipeTab === 'HALF' ? 'bg-white text-orange-700 shadow-sm' : 'text-slate-500 hover:text-orange-700'
                              }`}
                        >
                           Half Plate Recipe
                        </button>
                        ...
                        sortedCategoryKeys.map(category => {
                           ...
                           return (
                              <React.Fragment key={category}>
                                 {/* Group Header */}
                                 <tr
                                    className="bg-slate-50 curser-pointer hover:bg-slate-100 transition-colors cursor-pointer"
                                 ...
                                 {isExpanded && items.map((item, index) => (
                                    <tr key={item.id} id={`menu-row-${item.id}`} className="hover:bg-slate-50 transition-colors scroll-mt-32 border-b border-slate-100 last:border-0">
                                       ...
                                       <td className="p-4">
                                          {/* Category column is redundant now but requested to keep functionality logic - can keep or dim it */}
