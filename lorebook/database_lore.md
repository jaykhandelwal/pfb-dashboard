...
-- 10. SEED DATA (Essential Settings) ----------------------------------------
...
**Notable Keys for App Configuration:**
*   `enable_attendance_webhook`: Controls webhook firing for selfies.
*   `ledger_categories`: Array of active ledger categories.
*   `payment_methods`: Configured payment methods.
*   `ledger_accounts`: Definitions of source or destination funds.
...
## 5. Ledger Data (Financials)

### 17. `ledger_entries` (Financial Transactions)
*Unified ledger for income, expenses, and reimbursements.*
...
| `category` | `TEXT` | Display name of the ledger category |
| `category_id` | `TEXT` | Link to internal category ID |
| `amount` | `NUMERIC` | Transaction value |
| `description` | `TEXT` | Optional notes |
| `payment_method` | `TEXT` | Display name for payment |
| `payment_method_id` | `TEXT` | Link to internal method ID |
| `source_account` | `TEXT` | Origin of funds |
| `source_account_id` | `TEXT` | Origin ID |
| `destination_account` | `TEXT` | Target of funds (Reimbursement) |
| `destination_account_id` | `TEXT` | Target ID |
| `status` | `TEXT` | `PENDING`, `APPROVED`, `REJECTED` |
| `approved_by` | `TEXT` | User who audited |
| `rejected_reason` | `TEXT` | Notes on rejection |
| `bill_urls` | `TEXT[]` | Array of image URLs (BunnyCDN) |
| `linked_expense_ids` | `TEXT[]` | IDs of grouped entries |
| `created_by` | `TEXT` | User ID |
| `created_by_name` | `TEXT` | User Name |
| `timestamp` | `BIGINT` | Epoch ms |

### 18. `ledger_logs` (Audit Trail)
*Immutable logs tracking every approval or update made in the ledger.*

| Column Name | Type | Description |
| :--- | :--- | :--- |
| `id` | `TEXT` (Primary Key) | Log ID |
| `ledger_entry_id` | `TEXT` | Foreign Key to `ledger_entries` |
| `action` | `TEXT` | `CREATE`, `UPDATE`, `APPROVE`, `REJECT`, `DELETE` |
| `performed_by` | `TEXT` | Actor User ID |
| `performed_by_name` | `TEXT` | Actor Name Snapshot |
| `snapshot` | `JSONB` | Full state of `ledger_entries` at time of action |
| `date` | `TEXT` | Operation Date |
| `timestamp` | `BIGINT` | Epoch Ms |
| `created_at` | `TIMESTAMPTZ` | Autogenerated Timestamp |
