    ...
    <div className="flex h-screen bg-slate-50 text-slate-800 font-sans overflow-hidden">
      ...
      <aside className="w-64 bg-slate-900 text-white flex-shrink-0 hidden md:flex flex-col shadow-xl z-20">
        <div className="p-6 border-b border-white/10 flex flex-col items-center">
          <h1 className="text-3xl font-bold text-emerald-400 tracking-wider">PAKAJA</h1>
          <p className="text-xs text-slate-400 tracking-widest uppercase mt-1">Inventory Ops</p>
        </div>
        ...
        <div className="p-4 border-t border-white/10">
          <button 
            onClick={logout}
            className="flex items-center space-x-3 px-4 py-2 w-full text-slate-400 hover:text-red-400 hover:bg-white/5 rounded-lg transition-colors"
          >
            <LogOut size={20} />
      ...
      <div className="md:hidden fixed top-0 w-full z-50">
        {/* Header Bar */}
        <div className="bg-slate-900 text-white px-4 h-16 flex justify-between items-center shadow-md relative z-50">
           <div className="flex items-center gap-2">
              <span className="font-bold text-xl text-emerald-400 tracking-wide">PAKAJA</span>
           </div>
        ...
        {/* Dropdown Menu */}
        {isMobileMenuOpen && (
          <div className="absolute top-16 left-0 w-full bg-slate-800 shadow-2xl border-t border-slate-700 flex flex-col max-h-[85vh] overflow-y-auto animate-in slide-in-from-top-2 duration-200">
              {/* User Info (Mobile) */}
              ...
              {/* Logout (Mobile) */}
              <div className="p-4 border-t border-slate-700 mt-2 pb-6">
                  <button 
                      onClick={() => {
                          handleMobileNavClick();
                          logout();
                      }}
                      className="flex items-center justify-center space-x-2 px-4 py-3 w-full text-red-400 bg-slate-900/50 hover:bg-slate-700 rounded-lg transition-colors font-medium border border-slate-700"
                  >
                      <LogOut size={20} />
                      <span>Sign Out</span>
                  </button>
      ...
      {/* Main Content */}
      <main className="flex-1 overflow-y-auto w-full bg-slate-50 relative z-0">
